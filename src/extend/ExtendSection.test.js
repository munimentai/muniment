...
it('searches and filters the MCP catalog before displaying a page', async () => {
  ...
  await fireEvent.click(screen.getByRole('button', {name:'Category: All categories'}))
  await fireEvent.click(screen.getByRole('menuitemradio', {name:'Legal'}))
...
it('shows twelve popular cards before the rest and supports name sorting', async () => {
  ...
  await fireEvent.click(screen.getByRole('button',{name:'Filters'}))
  expect(screen.getByRole('button', {name:'Sort: Popularity'})).toBeInTheDocument()
  await fireEvent.click(screen.getByRole('button', {name:'Sort: Popularity'}))
  await fireEvent.click(screen.getByRole('menuitemradio', {name:'Name A–Z'}))
...
it('hides filters initially and opens custom setup as a dialog', async () => {
  ...
  await fireEvent.click(screen.getByRole('button',{name:'Close extension setup'}))
...
it('keeps installed MCPs in the catalog with a switch and detail controls', async () => {
  ...
  await fireEvent.click(screen.getByRole('button',{name:'Filters'}))
  await fireEvent.click(screen.getByRole('button', {name:'Show: All servers'}))
  await fireEvent.click(screen.getByRole('menuitemradio', {name:'Installed'}))
...
it('dismisses extension setup on backdrop click and Escape without saving', async () => {
  const invoke = vi.fn(async () => ({items:[]}))
  render(ExtendSection,{tauri:{invoke}})
  await waitFor(() => expect(screen.getByRole('button',{name:'Custom'})).not.toBeDisabled())
  await fireEvent.click(screen.getByRole('button',{name:'Custom'}))
  let dialog = await screen.findByRole('dialog',{name:'Add MCP server'})
  dialog.getBoundingClientRect = () => ({left:100,right:300,top:100,bottom:250})
  await fireEvent.pointerDown(dialog,{clientX:50,clientY:50})
