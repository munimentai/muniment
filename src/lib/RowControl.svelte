...
<button type="button" class="quiet row-control" class:new-thread={kind === 'new-thread'} class:thread-title={kind === 'thread-title'} class:artifacts-toggle={kind === 'artifacts-toggle'} class:record-toggle={kind === 'record-toggle'} bind:this={element} {...rest}>{@render children()}</button>
...
<style>
  ...
  .new-thread, .artifacts-toggle, .record-toggle { white-space: nowrap; }
  ...
  /* Artifacts then Record sit after the update slot on macOS, Record flush right in the row. */
  :global(.workspace.macos) .artifacts-toggle { order: 2; }
  :global(.workspace.macos) .record-toggle { order: 3; }
</style>
