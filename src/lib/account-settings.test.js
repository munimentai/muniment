...
const source = fs.readFileSync(path.join(process.cwd(), 'src/lib/AccountSettings.svelte'), 'utf8')
...
function renderPanel(invoke) {
  return render(AccountSettings, { props: {
    tauri: { invoke },
...
describe('contract entitlement snapshot', () => {
  it('shows the profile, capabilities, and allow and deny grants without resolving policy', async () => {
    ...
    renderPanel(invoke)
    await screen.findByText('Snapshot v42')
    const section = screen.getByRole('group', { name: 'Your access' })
  ...
  it('shows empty capabilities and grants without inventing access', async () => {
    ...
    expect(await screen.findByText('The snapshot lists no capabilities.')).toBeInTheDocument()
...
describe('account settings layout', () => {
  it('renders thread retention after appearance with the default choice selected', async () => {
    ...
    const sections = [...document.querySelectorAll('.account-sections > section')]
    expect(sections.slice(0, 2).map((section) => section.getAttribute('aria-labelledby'))).toEqual([
      'retention-heading',
  ...
  it('records a thread retention choice and restores it on a later read', async () => {
    ...
    // A later visit to the section reads the saved choice back.
    cleanup()
    renderPanel(invoke)
    await waitFor(() => expect(screen.getByRole('radio', { name: 'Delete after 90 days' })).toBeChecked())
  ...
  it('cancels a revoke with Escape and returns focus to the program', async () => {
    renderPanel(companionInvoke())
    ...
    expect(screen.getByRole('group', { name: 'Your access' })).toBeInTheDocument()
  })

  it('keeps the sections rendered when document receives Escape during a revoke', async () => {
    renderPanel(companionInvoke())
  ...
  it('updates the instance lock state when the background service stops presenting', async () => {
    let statusCalls = 0
    const invoke = vi.fn(async (command) => {
      if (command === 'auth_entitlement_snapshot') return snapshot
      if (command === 'auth_devices') return []
