...
describe('theme state', () => {
  it('names four light and four dark themes with a scheme each', () => {
    expect(LIGHT_THEMES).toEqual(['Paper', 'Vellum', 'Ledger', 'Foolscap', 'Parchment', 'Manila', 'Linen', 'Broadsheet', 'Daybreak', 'Coral'].map((name) => name.toLowerCase()))
    expect(DARK_THEMES).toEqual(['Moss', 'Vault', 'Graphite', 'Inkwell', 'Lagoon', 'Umber', 'Fjord', 'Plum', 'Nocturne', 'Nightshade', 'Basalt', 'Obsidian', 'Carbon', 'Afterglow', 'Canopy'].map((name) => name.toLowerCase()))
