...
describe('Appearance', () => {
  it('offers three modes and every theme, each with a tile of its own tokens', () => {
    ...
    expect(within(themes).getAllByRole('button').map((button) => button.textContent.trim())).toEqual([...['Paper', 'Vellum', 'Ledger', 'Foolscap', 'Parchment', 'Manila', 'Linen', 'Broadsheet'], ...['Moss', 'Vault', 'Graphite', 'Inkwell', 'Lagoon', 'Umber', 'Fjord', 'Plum', 'Nocturne', 'Nightshade', 'Basalt', 'Obsidian', 'Carbon', 'Daybreak', 'Afterglow']])
    expect([...themes.querySelectorAll('[data-swatch]')].map((swatch) => swatch.dataset.theme)).toEqual([...['Paper', 'Vellum', 'Ledger', 'Foolscap', 'Parchment', 'Manila', 'Linen', 'Broadsheet'], ...['Moss', 'Vault', 'Graphite', 'Inkwell', 'Lagoon', 'Umber', 'Fjord', 'Plum', 'Nocturne', 'Nightshade', 'Basalt', 'Obsidian', 'Carbon', 'Daybreak', 'Afterglow']].map((name) => name.toLowerCase()))
    ...
    for (const swatch of themes.querySelectorAll('[data-swatch]')) {
      ...
      expect([...swatch.querySelectorAll('i')].map((bar) => bar.classList[0])).toEqual(['swatch-ink', 'swatch-muted', 'swatch-signal', ...(['daybreak', 'afterglow'].includes(swatch.dataset.theme) ? ['swatch-accent'] : [])])
    }
