...
- NEXT E3 — add the Linux pure-core executable identity verifier: canonical expected and proc-owned paths, process-start revalidation against PID reuse, exact comparison, and redacted failures. Later slices map accepted loopback connections to OS process identity, add macOS/Windows identity adapters, compose the desktop loopback transport, implement the unpacked MV3 extension/connect-tab lifecycle, integrate runtime tools, enforce entitlement and ask/allow/deny policy (domain/read-vs-act/sensitive approval), write journal receipts, and wire the kill switch.
