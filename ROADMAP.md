...
- DONE E2 through live run rendering — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests, a first-page native Activity Bar thread list, the selected thread's first redacted page in a read-only native editor, typed idempotent `run.start`, a native New Run command with bounded prompt submission and sanitized status feedback, typed ordered `run.stream` delivery with cursor acknowledgements, and a live read-only run document using only redacted stream projections and authoritative receipts.
- NEXT E2 — attach the active non-empty editor selection to New Run only when its file belongs to the open workspace, using a workspace-relative identity and the existing bounded `run.start` context. Then proceed in separate slices with whole-file context and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
