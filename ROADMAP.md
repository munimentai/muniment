...
- DONE E2 through native run submission — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests, a first-page native Activity Bar thread list, the selected thread's first redacted page in a read-only native editor, typed idempotent `run.start`, and a native New Run command with bounded prompt submission and sanitized status feedback.
- NEXT E2 — add typed ordered `run.stream` delivery and cursor acknowledgements to the editor transport without UI changes. Then proceed in separate slices with live run rendering, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
