...
- NEXT — frontend-only shell slices, in order: `docs/spec/02-desktop-app.md` §4's auto-growing composer (the fixed `rows="2"` textarea grows to a 10-line cap, then scrolls internally), then `docs/spec/02-desktop-app.md` §3.2's quiet per-message action row, Copy only in this slice.
