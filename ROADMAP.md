...
- DONE E1 write protocol — the Linux attach session authorizes, bounds, and dispatches `run.start`, and the protocol-only Linux client sends correlated requests with fresh request/idempotency identities and strictly validates accepted receipts.
- NEXT E1 — extract a channel-neutral desktop run-start coordinator from the Tauri chat command so the attach service can invoke the same authorization, journal, active-run, and Pi-runtime path without duplicating behavior. Later slices implement the attach service adapter, add the interactive CLI send command, run streaming, and permission-answer parity.
