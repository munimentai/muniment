...
- DONE E2 through workspace-scoped selection context — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests, a first-page native Activity Bar thread list, the selected thread's first redacted page in a read-only native editor, typed idempotent `run.start`, a native New Run command with bounded prompt submission and sanitized status feedback, typed ordered `run.stream` delivery with cursor acknowledgements, a live read-only run document using only redacted stream projections and authoritative receipts, and active non-empty editor selection context restricted to files in the open workspace.
- DONE E2 context — an explicit New Run with Current File command sends the active workspace file's current buffer and workspace-relative identity through the existing bounded `run.start` context.
- DONE E2 permission transport — typed bounded `permission.pending` stream delivery and idempotent allow/deny `permission.answer` requests are available to the editor client.
- NEXT E2 — add a native in-editor allow/deny interaction for pending permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
...
- Later slices add macOS/Windows identity adapters, the unpacked MV3 extension/connect-tab lifecycle, runtime tools, entitlement and ask/allow/deny policy, journal receipts, and kill switch.
