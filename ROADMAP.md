...
- NEXT (selected 2026-07-25, second pass; independently implementable in any order) —
  - (a) **Defect, product inspection.** The signed-in workspace still paints the centered sign-in lockup and `shell v0.0.1` behind the conversation. `src/App.svelte:1231` sets `class:signed-frame`, but no `.signed-frame` rule has ever existed, so `main` keeps `place-content: center` under the fixed `.workspace` grid and the wordmark shows through the transparent thread column. Verified against the built bundle in headless Chromium at 1280×820: `main > .lockup` occupies `{x: 545, y: 368, w: 190, h: 47}` while `elementFromPoint(640, 400)` is a user bubble — the mark sits behind live conversation, in both themes, and does not scroll away. The owner mockup has no such watermark.
  - (b) **Design law.** The focus-ring drift at `src/lib/AccessPanel.svelte:228` (2px `--muted` at offset `-2`, clipped-group workaround) against `docs/spec/01-design-system.md:126`'s 2px ink at offset 2, plus widening the §1.2/§6 guard from `base.css` alone to every component, with `.artifact-divider` as the one documented exception.
  - (c) **Code health.** The `src/lib/Onboarding.svelte` extraction described below.
- SELECTED, now ticketed (code health) — `src/App.svelte` is 1832 lines carrying onboarding, auth, chat, dictation, sidebar, and the rail in one file; extract the onboarding surface (state 93–104, handlers 182–332, markup 1242–1349, styles 1600–1643) into `src/lib/Onboarding.svelte` following the `src/lib/AccessPanel.svelte` precedent, keeping every `data-testid` and accessible name so `src/App.test.js`'s onboarding suite passes unchanged. The couplings to respect are the sidebar's `Home settings` write at `1381`, App's own onboarding gating at `937`/`941`/`1030`/`1035`/`1241`, the `onMount` load/teardown at `1008`/`1087`, and the `.eyebrow` / `.primary` / `.support` / base `button` rules shared with the rail, composer and signed-out screen.
...
- OBSERVED, NOT YET SELECTED (design-spec audit 2026-07-25) — the streaming underline is applied to the whole response paragraph (`src/App.svelte:1406`, `:1735`), so a wrapped reply paints a signal border under every line rather than §2.2's active line; the ring's thinking state is a fixed opacity fade rather than §1.8's decorrelated breath/spin/trace with a completing done-gesture, and `src/lib/mark.js` reads a fixed path so milling depth cannot flex; `--shadow-*` and `--space-*` tokens are absent while `--radius-panel` and `--text-17/22/28` are declared and never used, with twelve raw radius values in their place (two of them `8px`, off §1.5's 2/6/10 scale); responses lack §2.2's 92% max-width; the titlebar has no `⌘K` hint, and should not get one while the palette itself is deferred. The streaming-underline and ring items still need a design call on "active line" and on how much of the reference engine to port before they are ticketable.
...
- OBSERVED 2026-07-25 — the signed-in-frame defect above survived every layer of this suite, because the nightly screenshots deliberately exclude rendered production conversations and the frontend suite asserts presence rather than absence. A rendered-frame check against a stubbed signed-in workspace (built bundle, headless Chromium) would have caught it; consider one after the fix lands, rather than widening what the nightly uploads.
