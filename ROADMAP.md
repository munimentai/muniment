...
- DONE E1 stream server groundwork — the authorized Linux attach session serves bounded, redacted journal-backed `run.stream` catch-up with workspace isolation, sequence/window validation, and an explicit caught-up event.
- DONE E1 protocol client catch-up — the protocol-only client sends typed `run.stream` subscription requests and validates ordered, bounded, redacted catch-up events through the explicit caught-up marker.
- NEXT E1 — retain server-side subscription flow-control state and handle `run.cursor_ack` so a window-paused catch-up can continue. Later independent slices add client acknowledgements/live tail, terminal rendering, and permission-answer parity.
