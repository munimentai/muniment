...
- DONE E2 groundwork, native transport, read client, thread list UI, and selected-thread preview — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests over the authorized connection, a first-page native Activity Bar thread list, and the selected thread's first redacted page in a read-only native editor.
- NEXT E2 — add a typed, fixture-pinned `run.start` operation to the authorized editor transport as composer groundwork. Then proceed independently with the composer command/UI, run streaming, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
