...
DONE 2026-08-28 — the locked bind, registration diagnostics, and Windows
runtime start record landed (MUNIDESK-1518, 1519, and 1520).
`WindowsAttachListener::bind`
(`src-tauri/core/src/attach/windows_listener.rs:147`) acquires the per-profile
instance lock before it creates the first pipe instance. The listener holds the
lock for its lifetime, so the ADR 0012 attach admission gate is open.
`register_runtime_task` (`src-tauri/src/windows_runtime_service.rs:62`) records
both unavailable and timed-out install locks as
`event=install_lock_unavailable message=install lock unavailable`. It records
`NoInstalledPayload`, refused, and failed outcomes as
`event=runtime_task_registration_failed message=runtime task registration failed`.
Registered, updated, and unchanged outcomes write nothing. Windows `main`
(`src-tauri/runtime/src/main.rs:74`) now calls `run_recorded_windows_activation`,
...
DONE 2026-08-29 — the bind-and-serve activation step, the platform-neutral
client stream, the startup worker thread, and the connection-route amendment
landed (MUNIDESK-1536, 1537, 1538, and 1539). `run_windows_attach_activation`
(`src-tauri/runtime/src/windows_attach_activation.rs:35`) binds an acceptor
through a factory. It maps a contended instance lock to `Orderly(0)` with the
`InstanceLockWait` diagnostic, and it maps every other bind failure to
`Failed(1)` with the `ActivationFailed` diagnostic. `SystemWindowsAttachFactory`
(`:58`) is the production factory, and it has no caller. `DesktopClient`
(`src-tauri/attach/src/client.rs:2155`) holds a boxed `ClientStream`, and
`muniment-core` implements that trait for `WindowsAttachStream`
(`src-tauri/core/src/attach/windows_stream.rs:162`). The Tauri setup hook
(`src-tauri/src/main.rs:57`) spawns one worker thread for the Windows
registration and start, so the first window waits for neither. ADR 0012 carries
the Windows attach connection route amendment
(`docs/decisions/0012-user-level-runtime-service.md:1198`). It names
...
(`src-tauri/attach/src/client.rs:2155`) and its whole impl block name no
`UnixStream`, so the type itself is portable. Its handshake and request path
still call `deadline`, `fresh_request_id`, `fresh_nonce`, `is_hex_secret`,
`map_frame_error`, `map_protocol_error`, `parse_message`, and
`reject_protocol_error`. All eight live inside `#[cfg(unix)] mod linux` (`:497`),
so Windows gets the `UnsupportedPlatform` stub (`:4307`). `random_bytes`
(`:3990`) also reads `/dev/urandom`, which Windows does not have. The lane
therefore moves those helpers into a platform-neutral module and takes
randomness from `getrandom`, which `muniment-core` already depends on. The
`DesktopClient` move follows in a later slice.
...
It mirrors the Linux layout, where `INSTANCE_LOCK_NAME`
(`src-tauri/core/src/attach/linux.rs:80`) sits in the attach directory beside the
endpoint. Core cannot read `%APPDATA%` itself, because `windows_state_directory`
lives in the runtime crate (`src-tauri/runtime/src/directories.rs:71`). The
caller therefore injects the state directory.

DECIDED 2026-08-26 (planner, read the Microsoft named-pipe impersonation
reference against the byte-mode endpoint the listener creates) — the Windows
listener reads the four-byte frame-length prefix before it verifies the peer.
`ImpersonateNamedPipeClient` fails with `ERROR_CANNOT_IMPERSONATE` on a byte-mode
pipe until the server reads from that pipe. The call adopts the security context
of the last message read. The 2026-08-22 amendment orders the peer check before
any frame read (`docs/decisions/0012-user-level-runtime-service.md:1116`), so the
...
`windows-sys` 0.61 exposes functions, constants, and structs alone, so it
declares no `ITaskService`, `ITaskFolder`, or `IRegisteredTask` interface.
`Cargo.lock` already pins `windows` 0.61.3 for the desktop tree, and
...
composes the whole service: journal and CAS, device session, entitlement
snapshot, credentials, workspace authorization, permission gates, run control,
thread authority, the chat event broadcast, and the ADR 0009 listener.
`run_runtime_activation` (`src-tauri/runtime/src/activation.rs:40`) serves the
attach endpoint when `main` starts, takes the endpoint from a lock-holding
desktop through the nonce-bound migration takeover, and the Linux package
installs `/usr/lib/systemd/user/muniment-runtime.service` with postinst
enablement (MUNIDESK-1334). On Linux the chat, thread, auth, and companion
commands ride the connected desktop client through `DesktopClientHolder`
(`src-tauri/attach/src/client.rs`), a desktop that meets a held instance lock
opens no journal and marks no run interrupted (MUNIDESK-1333), runtime
...
(`src-tauri/attach/src/client.rs:1466`) records the welcome version.
`with_compatible_client` (`:1636`) answers `RuntimeUpgradePending` before it
sends a run operation, so the four run commands hold while the runtime is
older. `runtimeUpgradePending` (`src/App.svelte:855`) holds send, resume, and
queue in the shell, and it shows the pending upgrade notice.

DONE 2026-08-18 — the runtime unit states its restart backoff (MUNIDESK-1364).
`src-tauri/packaging/muniment-runtime.service` carries `RestartSec=5s` under
`[Service]`, and `StartLimitIntervalSec=300s` with `StartLimitBurst=5` under
`[Unit]`. The unit no longer takes the `systemd` defaults of a 100ms delay and
five starts in 10 seconds. A crash loop now reaches that limit in about twenty
seconds and leaves the unit failed, which is the needs-attention state ADR 0012
asks for. The status-75 upgrade refresh spends one of the five starts
...
client, and no desktop command rides the wire for it, because onboarding
scaffolds the Home through local file writes alone. The composition-time Home
concern moved to the eighty-second-wave measurement above.
...
DONE 2026-08-18 — a momentary background-service drop leaves the workspace on
screen (MUNIDESK-1368). `createBackgroundServiceNotice`
(`src/lib/background-service-notice.js:4`) waits out a two-second dwell before
it raises the notice. The notice branch gates on
`backgroundServiceNoticeVisible` (`src/App.svelte:906`) rather than on
`connected` or `chat_events_connected` alone. The `Muniment cannot reach its
background service.` notice (`:908`) still replaces the workspace (`:912`) for
an outage that outlasts the dwell. The eighty-sixth wave had measured every
drop unmounting the whole workspace. The remount cost the transcript its scroll
position, and the focus effect (`:628`) took composer focus back (`:633`). The
runtime drops a chat-event subscriber whose 256-event queue fills
(`src-tauri/runtime/src/sink.rs:62`), and the desktop reconnects after a 250
millisecond retry interval (`src-tauri/src/attach_service.rs:1144`).
