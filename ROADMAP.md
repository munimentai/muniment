...
- DONE E2 groundwork and native transport — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, and bounded Linux/macOS Unix-socket plus Windows named-pipe discovery, framing, pairing, and authorization.
- NEXT E2 — retain the authorized transport and add a typed, bounded read-only `thread.list`/`thread.open` client as one UI-neutral slice. Then proceed independently with the native thread list/view and composer, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
