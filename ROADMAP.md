...
- DONE — ADR 0006, verified revision publication/recovery, bounded resumable acquisition, the shared native HTTPS/proxy acquisition transport, and the pure-core cancellable-lock/free-space install coordinator.
- NEXT — exact model-specific resumable-stage byte accounting, then wire Gemma acquisition/publication through the coordinator. Activation-health rollback, native adapters/Tauri commands, UI, and release notice delivery follow as separate slices.
...
- DONE — ADRs 0004/0005, pinned Parakeet verification, revision publication/recovery, bounded resumable acquisition, consumption of the shared native HTTPS/proxy transport, and the shared pure-core install coordinator.
- NEXT — exact model-specific resumable-stage byte accounting, then wire Parakeet acquisition/publication through the coordinator. Native adapters/Tauri commands, UI, native bindings/packaging, capture/VAD, and hardware validation follow separately.
