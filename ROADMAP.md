...
- DONE E2 through run-stream transport — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests, a first-page native Activity Bar thread list, the selected thread's first redacted page in a read-only native editor, typed idempotent `run.start`, a native New Run command with bounded prompt submission and sanitized status feedback, and typed ordered `run.stream` delivery with cursor acknowledgements.
- NEXT E2 — render an accepted run live in a read-only native editor document, using only the redacted stream projection and authoritative receipt. Then proceed in separate slices with workspace-scoped editor context and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
