...
- PARKED WITH A HUMAN 2026-07-25 — `docs/spec/02-desktop-app.md` §4's auto-growing composer and §3.2's quiet per-message Copy action row both stalled in implementation and now sit in Needs Human. They remain the right slices and keep their spec'd scope, but neither is re-selectable until a human resolves them; do not re-file either.
- NEXT — design-spec fidelity slices, audited against the written spec on 2026-07-25 and independently implementable in any order: (a) §1.2's color LAW — focus rings move from `--signal` to 2px ink at offset 2, at-rest badges/dots/chips drop the accent, and §7's promised signal-allowlist lint lands as a vitest guard; (b) §2.3/§4's ink Send button plus §2.6/§10's composer-focused-on-entry; (c) §1.3/§2.1's per-device System/Light/Dark override in the profile popover (both palettes already exist; only the user's choice is missing).
...
- OBSERVED, NOT YET SELECTED (design-spec audit 2026-07-25) — the streaming underline is applied to the whole response paragraph (`src/App.svelte:1289`, `:1610`), so a wrapped reply paints a signal border under every line rather than §2.2's active line; the ring's thinking state is a fixed opacity fade rather than §1.8's decorrelated breath/spin/trace with a completing done-gesture, and `src/lib/mark.js` reads a fixed path so milling depth cannot flex; `src/styles/base.css` carries none of the spec'd global element defaults (`::selection`, link, heading, mono-numeral, blanket reduced-motion) that the mockup design-system bundle defines, so reduced-motion compliance rests on a hand-maintained five-item list at `src/App.svelte:1673`; `--shadow-*`, `--motion-*`, `--weight-*`, `--tracking-*` and `--space-*` tokens are absent while `--text-17/22/28` and `--radius-panel/chip` are declared and never used, with several off-scale raw px values in their place; responses lack §2.2's 92% max-width; the titlebar has no `⌘K` hint. The streaming-underline and ring items need a design call on "active line" and on how much of the reference engine to port before they are ticketable.
- SELECTED FOR A LATER WAVE (code health) — `src/App.svelte` is 1674 lines carrying onboarding, auth, chat, dictation, sidebar, and the rail in one file; extract the ~309-line onboarding surface (script 87–91/98/172–322, markup 1118–1226, styles 1467–1508) into `src/lib/Onboarding.svelte` following the `src/lib/AccessPanel.svelte` precedent, keeping every `data-testid` and accessible name so `src/App.test.js:413-863` passes unchanged. Still held back: the two parked shell slices above land in this same file when a human unblocks them, and a 300-line extraction would renumber it underneath them.
