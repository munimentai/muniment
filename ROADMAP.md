...
files, `open_selected_files` (`src-tauri/src/chat.rs:1069`) validates each open
...
`src-tauri/core/src/memory_scan.rs`, `src-tauri/core/src/attachment.rs`, and
`src/App.svelte`. Two open slices edit `src-tauri/src/chat.rs` and
`src-tauri/core/src/attachment.rs` this wave. Each ticket tells the implementer
...
planner then read the whole attachment path, meaning the composer selection, the
CAS ingest, the journal event, the reducer projection, the Pi image delivery, and
the transcript chips. It also read the resume command and its core module. It
rebuilt the bundle and captured the empty workspace, restored history, Markdown,
and signed-out fixtures in headless Chromium at 1100x760, plus restored history
at the 960x640 window minimum. Earlier waves recorded the same shape of
