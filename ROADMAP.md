...
- DONE E2 groundwork, native transport, read client, and first list UI — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests over the authorized connection, and the first-page native Activity Bar thread list with explicit loading/pairing/empty/failure states.
- NEXT E2 — render the selected thread's first redacted page as one read-only, presentation-only slice. Then proceed independently with the composer, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
