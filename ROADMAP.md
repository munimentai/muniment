...
- NEXT E2 — add typed `permission.pending` delivery and `permission.answer` requests to the editor attach transport as groundwork for a separate in-editor allow/deny interaction slice. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
