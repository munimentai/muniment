...
- DONE E2 groundwork, native transport, and read client — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, and typed bounded `thread.list`/`thread.open` requests over the authorized connection.
- NEXT E2 — add the first-page native Activity Bar thread list as one presentation-only slice. Then add selected-thread rendering before proceeding independently with the composer, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
