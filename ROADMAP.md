...
- DONE E1 foundation — the Rust workspace, path-scoped CLI CI lane, protocol-only `muniment-attach` crate dependency boundary, Linux CLI discovery plus explicit pairing handshake, and one-page journal-backed `muniment threads list` over the preserved authorized connection are present.
- NEXT E1 — add interactive cursor navigation to `muniment threads list`. Later slices add thread read, send, run stream, and permission-answer parity. E2 may proceed concurrently in its independently scoped TypeScript package lane per ADR 0011; VSIX sideload testing is sufficient and marketplace publication remains owner-gated.
