...
- NEXT — frontend-only shell slices, in order: `docs/spec/02-desktop-app.md` §4's auto-growing composer (the fixed `rows="2"` textarea grows to a 10-line cap, then scrolls internally); `docs/spec/02-desktop-app.md` §3.2's quiet per-message action row, Copy only in this slice; and provenance-line fidelity against the owner mockup (`route → model · cost · time`, `--signal` bound to the route field rather than to whichever part is first, tabular numerals, 11.5px).
...
- SELECTED FOR A LATER WAVE (code health) — `src/App.svelte` is 1670 lines carrying onboarding, auth, chat, dictation, sidebar, and the rail in one file; extract the ~11-state onboarding surface into `src/lib/Onboarding.svelte` following the `src/lib/AccessPanel.svelte` precedent. Held back this wave so it does not collide with the three shell slices above.
