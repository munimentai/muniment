...
- NEXT E2 — add an explicit New Run with Current File command that sends the active workspace file's current buffer and workspace-relative identity through the existing bounded `run.start` context. Then proceed in a separate slice with in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
