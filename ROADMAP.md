...
- DONE E1 write groundwork — the Linux attach session authorizes, bounds, and dispatches `run.start` through an injected service seam with request/idempotency identity and companion provenance.
- DONE E1 write protocol — the protocol-only Linux client sends correlated `run.start` requests with fresh request/idempotency identities and strictly validates accepted receipts.
- DONE E1 shared launch path — the Tauri chat command delegates run acceptance to an injected, channel-neutral desktop coordinator that owns validation, authorization, journal preparation, active-run installation, and Pi launch.
- NEXT E1 — implement the production attach service adapter that invokes the shared desktop run-start coordinator and returns the committed acceptance receipt without duplicating behavior. Later slices add the interactive CLI send command, run streaming, and permission-answer parity.
