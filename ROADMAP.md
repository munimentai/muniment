...
- NEXT — frontend-only shell slices, in order: design-spec §4's auto-growing composer (the fixed `rows="2"` textarea grows to a 10-line cap, then scrolls internally); design-spec §3.2's quiet per-message action row, Copy only in this slice; and provenance-line fidelity against the owner mockup (`route → model · cost · time`, `--signal` bound to the route field rather than to whichever part is first, tabular numerals, 11.5px).
