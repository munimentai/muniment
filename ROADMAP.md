...
- DONE E0–E10 — extension-only real-profile architecture; attributed upstream-mergeable relay core; fail-closed single-use pairing; Linux executable identity and loopback socket-owner verification; composed authorization; authorized loopback listener; bounded RFC 6455 upgrade; pairing-token consumption; fail-closed macOS loopback browser-process identity; and macOS authorized-listener composition preserving the shared transport and handshake invariants.
- NEXT — add fail-closed Windows loopback browser-process identity as its own bounded slice before composing the Windows listener.
- Later slices add Windows listener composition, the unpacked MV3 extension/connect-tab lifecycle, runtime tools, entitlement and ask/allow/deny policy, journal receipts, and kill switch.
