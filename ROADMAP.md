...
- DONE E2 groundwork, native transport, read client, thread list UI, selected-thread preview, and run-start client — Rust-owned canonical `muniment.attach/1` fixtures, deterministic drift checking, fixture-pinned TypeScript package/test/package CI, bounded Linux/macOS Unix-socket plus Windows named-pipe discovery/framing/pairing/authorization, typed bounded `thread.list`/`thread.open` requests over the authorized connection, a first-page native Activity Bar thread list, the selected thread's first redacted page in a read-only native editor, and a typed idempotent `run.start` operation.
- NEXT E2 — expose `run.start` through a bounded native composer command/UI. Then proceed independently with run streaming, workspace-scoped editor context, and in-editor permission gates. VSIX sideload testing is sufficient; marketplace publication remains owner-gated.
