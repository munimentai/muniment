...
- NEXT — the macOS install/launch smoke per ADR 0013 (pinned `.app.zip`, `/Applications` install, healthy-first-window assertion, Proxmox screendump; no WDIO, no automated sign-in), and stable JUnit report parity for the Windows lane (`windows-e2e-report`). Chat/attachment flows and automatic failure-to-ticket reporting follow as separate slices.
