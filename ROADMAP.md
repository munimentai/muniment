...
- NEXT — desktop first-run slice: the unskippable directory picker plus visible Home scaffold (ticket filed 2026-07-20). Later slices: required-download visibility/status UI, consent-gated import preview/triage/report, and the CLI/VS Code inherit-opened-directory defaults once workspace memory semantics exist for those surfaces to point at.
