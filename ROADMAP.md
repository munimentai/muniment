...
- NEXT (selected 2026-07-26; (a) and (b) are file-disjoint from each other and may land in either order after the in-flight (c) lint, but both are sequenced behind (c): (a) overlaps its `revoked` work in `src/lib/AccessPanel.svelte`, and (b) overlaps its `voice`/`attach` work in `src/App.svelte`) —
...
- SEQUENCED BEHIND (c), NOT YET FILED — §1.4's type scale carries the same drift §1.2 and §1.5/§4 just had: 16 raw numeric font declarations across the three components — seven in `App.svelte` (30, 18, 10, 12.5, 11.5, 11, 11px), six in `AccessPanel.svelte` (18, 11, 10, 11, 11, 11px), and three in `Onboarding.svelte` (28, 13, 13px) — while the current scale is 12/13/15/17/22/28px and `--text-17` is declared but unused. The sweep covers all 16 declarations: values that duplicate or should resolve to the approved scale become token references, and any size that the spec requires outside that scale becomes a named type token rather than a raw-value lint exception (in particular, the required 11.5px mono provenance register gets its own token). The resulting lint permits component `font-size` declarations through type tokens only. It wants the same lint the colour and shape laws now have, but it rewrites font declarations in the same component style blocks the open class-usage lint is adding rules to, so it follows that ticket rather than running beside it.
