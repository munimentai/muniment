...
- NEXT E1 — add the matching bounded `run.start` request/response method to the protocol-only Linux attach client. Later slices connect the server seam to the desktop-owned runtime, add the interactive CLI send command, run streaming, and permission-answer parity.
