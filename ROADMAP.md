...
- DONE 2026-07-21 — first-run Home picker/scaffold; bounded ZIP preview and manifest review; bounded extraction of explicitly selected entries with verbatim text and stable provenance, no Home writes or model calls.
- NEXT — consent checklist UI and Tauri extraction bridge, retaining approved content only in transient pre-triage state (ticket filed 2026-07-21). Local-model triage, user-confirmed report, and import/scaffolding writes follow separately. Required-download visibility/status UI remains. CLI/VS Code defaults wait for workspace memory semantics.
