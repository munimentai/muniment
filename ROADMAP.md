...
## M0 — Scaffold
- DONE 2026-07-09 — Tauri v2 desktop shell, specs, mockups, design reference, Rust/frontend test harnesses, and Linux/Windows/macOS CI gates.
...
- DONE 2026-07-22 — desktop dictation polish, four post-dictation transforms, system-wide hold-to-talk, hands-free double activation, and user-rebindable global voice shortcuts.
- DONE 2026-07-22 — ADR 0015 pins the CPU-only Kokoro runtime, immutable model/voice artifacts, deterministic English text-processing and segmentation boundary, packaging contract, cancellation/privacy rules, and target-hardware validation gates.
...
- NEXT — implement ADR 0015's pure-core initial source-text packing pass against an injected phoneme/token-count boundary. The sub-20-token repair pass follows as a separate slice. Native runtime/G2P packaging remains subject to ADR 0015's redistribution approval gate; synthesis, playback, response controls, and physical target-hardware runs follow separately.
...
## Onboarding, memory, and the Muniment Home
- RATIFIED 2026-07-19 — visible human-editable Markdown Home; unskippable first-run directory picker; nearest-`AGENTS.md` semantics; consent-gated imports with preview, provenance, verbatim originals, and confirmation before writes; required local Qwen triage; versioned/eval-gated base prompt.
- DONE — specification, Home picker/scaffold, bounded ZIP preview/extraction, typed local-model triage, report review/confirmation, deterministic bounded Home write-plan compilation, and first-use local-model readiness UI.
- NEXT — persist a confirmed bounded write plan conflict-safely into Home. This work is in Needs Human and must not be re-filed until resolved. Desktop/Tauri completion wiring follows separately; CLI/VS Code defaults wait for workspace memory semantics.
- Open owner gates: headless/server CLI stance; final model promotion after MUNIQA routing eval; Gemma 4 E2B contingency pending llama.cpp PLE support; MUNICLOUD artifact proxy/redirect ripple.

## Desktop QA automation
- RATIFIED — layered frontend browser coverage plus installed-nightly real-app validation on serialized pve01 desktop-ci VMs.
- DONE — ADR 0013; canonical runner; Linux `.deb`, Windows MSI, and macOS install/launch coverage; Linux/Windows production-authenticated chat receipt checks; stable JUnit artifacts; SHA-scoped failure triage issues with private redacted diagnostics.
