...
- OBSERVED 2026-07-26 (same render) — §3.1's user bubble ships without its 78% max-width: a paragraph-length prompt fills the message column edge-to-edge (measured bubble width == column width, computed `max-width: none`), erasing the user/model asymmetry. And the artifact rail header doubles its label — mono `Thread artifacts` eyebrow directly over the `Artifacts` h2, the §2.1 doubled-heading shape, redundant a third time with the titlebar toggle. Selected together as one register slice.
- OBSERVED 2026-07-26 — §1 platform chrome: `src-tauri/tauri.conf.json` declares no `minWidth`/`minHeight` (spec: 960×640) and nothing restores window size/position, so every launch reopens at 1100×720 wherever the OS drops it; only the sidebar third of §1's restore sentence has shipped. Selected as a native slice via the official `tauri-plugin-window-state` (size-only restore is acceptable on Wayland, which does not report outer position).
- SELECTED 2026-07-26 — next code-health slice: the dictation session controller (capture epochs, status polling, quiet-period completion, polish, transform expiry — the largest remaining inline subsystem in the 1496-line `App.svelte`) moves into an injectable `src/lib/dictation-controller.js` in the `voice-shortcut.js` idiom with direct unit tests; the pointer/key gesture layer stays in `App.svelte` for a possible later slice.
- OBSERVED, NOT YET SELECTED — `--space-*` tokens are absent, and a spacing sweep would touch nearly every declaration in the app, so it stays parked rather than half-done. The titlebar has no `⌘K` hint, and should not get one while the palette itself is deferred. The full §1.8 ring behavior engine (decorrelated breath/spin/trace, flexing milling depth) is explicitly gated on a ≥34px thinking surface per the 2026-07-26 design call. The titlebar's static `New thread` session name (spec: editable inline) waits on the thread/branching contract — the desktop journal models runs, not threads (`list_threads` in `src-tauri/src/chat.rs` still answers `unsupported_operation`), so real thread listing/naming/switching is a contract-level lane that needs its own design decision before any UI slice.
