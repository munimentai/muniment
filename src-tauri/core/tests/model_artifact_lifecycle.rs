...
use muniment_core::model_artifact::lifecycle::{
    ModelArtifactActivation, ModelArtifactActivationBoundary, ModelArtifactActivationFailure,
    ModelArtifactLifecycleBoundary, ModelArtifactLifecycleError, ModelArtifactNoticeDescriptor,
    ModelArtifactPersistenceError, ModelArtifactRecovery, ModelArtifactRevisionDescriptor,
    ModelArtifactRevisionLifecycle, ModelArtifactUnavailable,
};
use muniment_core::model_artifact::ModelArtifactDescriptor;

static OLD_MODEL: ModelArtifactDescriptor = ModelArtifactDescriptor {
...
static NEW_MODEL: ModelArtifactDescriptor = ModelArtifactDescriptor {
    name: "artifact-fixture",
    version: "new",
...
static NEW: ModelArtifactRevisionDescriptor = ModelArtifactRevisionDescriptor {
    model: &NEW_MODEL,
    notice: ModelArtifactNoticeDescriptor {
        filename: "NOTICE.txt",
...
impl ModelArtifactLifecycleBoundary for Boundary {
    type LockGuard = ();
    fn lock_exclusive(&self, path: &Path) -> Result<(), ModelArtifactPersistenceError> {
...
impl ModelArtifactActivationBoundary for ActivationBoundary {
    ...
    fn launch(&self, model: &Path) -> Result<Self::Server, ModelArtifactActivationFailure> {
        ...
        if matches!(attempt, Attempt::StartFails) {
            Err(ModelArtifactActivationFailure::Start)
        } else {
    ...
    fn await_ready(&self, server: &mut Self::Server) -> Result<(), ModelArtifactActivationFailure> {
        match server.1 {
            Attempt::Ready => Ok(()),
            Attempt::ExitsBeforeReady => Err(ModelArtifactActivationFailure::ExitedBeforeReady),
            Attempt::ReadinessFails => Err(ModelArtifactActivationFailure::Readiness),
            Attempt::StartFails => unreachable!(),
        }
    }
}

fn published_update(root: &Path) -> ModelArtifactRevisionLifecycle {
    ModelArtifactRevisionLifecycle::new(root.to_owned(), &KNOWN, &OLD)
        .unwrap()
        .publish(&stage(root, "old-activation", b"abc"), &Boundary::working())
        .unwrap();
    let update = ModelArtifactRevisionLifecycle::new(root.to_owned(), &KNOWN, &NEW).unwrap();
    update
...
#[test]
fn rollback_restores_the_installed_previous_artifact_without_a_download() {
    let root = root();
    let lifecycle = published_update(&root);
...
fn failed_current_without_valid_previous_is_typed_unavailable() {
    let root = root();
    let lifecycle = ModelArtifactRevisionLifecycle::new(root.clone(), &KNOWN, &NEW).unwrap();
    lifecycle
    ...
    assert!(matches!(
        lifecycle
            .activate(&Boundary::working(), &activation)
            .unwrap(),
        ModelArtifactActivation::Unavailable(ModelArtifactUnavailable::PreviousInvalid)
    ));
    assert_eq!(activation.launched.lock().unwrap().len(), 1);
    assert_eq!(
        fs::read_to_string(root.join("activation-failure")).unwrap(),
...
fn failed_rollback_activation_stops_after_exactly_two_attempts() {
    ...
    assert!(matches!(
        lifecycle
            .activate(&Boundary::working(), &activation)
            .unwrap(),
        ModelArtifactActivation::Unavailable(ModelArtifactUnavailable::RollbackActivationFailed)
    ));
    assert_eq!(activation.launched.lock().unwrap().len(), 2);
    assert_eq!(
        lifecycle.resolve_current().unwrap(),
...
fn interrupted_activation_state_replacement_never_relaunches_rejected_current() {
    for interrupted_pointer in ["rejected", "activation-failure", "current"] {
        ...
        assert_eq!(
            ...
            Err(ModelArtifactLifecycleError::Persistence(
                ModelArtifactPersistenceError::Failed
            ))
        );
...
fn notice_must_match_the_revision_descriptor_for_publication_and_resolution() {
    let root = root();
    let lifecycle = ModelArtifactRevisionLifecycle::new(root.clone(), &KNOWN, &OLD).unwrap();
    ...
    assert_eq!(
        ...
        ModelArtifactRecovery::RepairRequired
    );
    fs::remove_dir_all(root).unwrap();
}
...
fn interrupted_pointer_replace_keeps_verified_current_and_previous() {
    ...
    let update = ModelArtifactRevisionLifecycle::new(root.clone(), &KNOWN, &NEW).unwrap();
    assert_eq!(
...
fn rejects_linked_stage_directory_and_model() {
    ...
    assert_eq!(
        ...
        Err(ModelArtifactLifecycleError::InvalidStage)
    );
...
fn linked_pointer_files_require_repair_even_when_the_target_looks_valid() {
    ...
    for external_contents in [
        "muniment-model-artifact-pointer-v1\nartifact-fixture\nold\n",
        "malformed",
    ] {
        let root = root();
        let lifecycle = ModelArtifactRevisionLifecycle::new(root.clone(), &KNOWN, &OLD).unwrap();
        ...
        assert_eq!(
            lifecycle.resolve_current(),
            Err(ModelArtifactLifecycleError::InvalidPointer)
        );
        assert_eq!(
            lifecycle.recover(&Boundary::working()).unwrap(),
            ModelArtifactRecovery::RepairRequired
...
fn unknown_or_unsafe_pointers_require_repair_without_leaking_paths() {
    ...
    assert_eq!(
        lifecycle.recover(&Boundary::working()).unwrap(),
        ModelArtifactRecovery::NotInstalled
    );
    fs::remove_dir_all(root).unwrap();
}
