...
fn verify(
    ...
    starts: impl IntoIterator<Item = Result<u64, ProcReadError>>,
    executable: Result<PathBuf, ProcReadError>,
) -> Result<muniment_core::attach::AuthorizedMigrationControlPeer, PeerAuthorityError> {
...
fn verify_presenter(
    ...
    verify_approval_presenter_peer_with_reader(
        424242,
        expected,
        &FakeProcReader::new([Ok(7), Ok(7)], Ok(executable)),
    )
}
...
fn rejects_relative_and_unresolvable_expected_paths() {
    ...
    assert_eq!(
        ...
        Err(PeerAuthorityError::ExpectedExecutableInvalid)
    );
    assert_eq!(
        verify(
...
fn rejects_unresolvable_and_deleted_peer_executables() {
    ...
    for executable in [
        ...
        assert_eq!(
            ...
            Err(PeerAuthorityError::PeerExecutableInvalid)
        );
    }
}
...
fn rejects_process_disappearance_and_identity_change() {
    ...
    assert_eq!(
        ...
        Err(PeerAuthorityError::PeerUnavailable)
    );
    ...
    assert_eq!(
        ...
        Err(PeerAuthorityError::PeerIdentityChanged)
    );
}
...
#[test]
fn presenter_accepts_the_expected_executable() {
    let executable = fs::canonicalize("/proc/self/exe").unwrap();

    assert!(verify_presenter(&executable, executable.clone()).is_ok());
}

#[test]
fn presenter_rejects_another_executable() {
    let expected = fs::canonicalize("/proc/self/exe").unwrap();
    let other = fs::canonicalize("/proc/self/status").unwrap();

    assert_eq!(
        verify_presenter(&expected, other),
        Err(PeerAuthorityError::ExecutableMismatch)
    );
}

#[test]
fn presenter_rejects_a_relative_expected_executable() {
    let executable = fs::canonicalize("/proc/self/exe").unwrap();

    assert_eq!(
        verify_presenter(Path::new("relative"), executable),
        Err(PeerAuthorityError::ExpectedExecutableInvalid)
    );
}
...
fn errors_are_bounded_and_redacted() {
    for error in [
        PeerAuthorityError::ExpectedExecutableInvalid,
        PeerAuthorityError::PeerUnavailable,
        PeerAuthorityError::PeerIdentityChanged,
        PeerAuthorityError::PeerExecutableInvalid,
        PeerAuthorityError::ExecutableMismatch,
    ] {
