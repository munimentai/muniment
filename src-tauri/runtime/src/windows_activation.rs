...
pub fn write_windows_diagnostic(
    ...
    let directory = windows_log_directory_from_local_app_data(local_app_data.as_ref())
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidInput, error))?;
    windows_file::append(local_app_data.as_ref(), &directory, event.record())
}
...
mod windows_file {
    ...
    use windows_sys::Win32::Foundation::{
        GetLastError, LocalFree, ERROR_ALREADY_EXISTS, ERROR_INSUFFICIENT_BUFFER, GENERIC_READ,
    ...
    impl OwnerSecurity {
        fn new() -> io::Result<Self> {
            ...
            if unsafe { InitializeAcl(acl.as_mut_ptr().cast(), acl_length as u32, ACL_REVISION) }
                == 0
                || unsafe {
    ...
    pub fn append(root: &Path, directory: &Path, record: &[u8]) -> io::Result<()> {
    ...
    fn open_owner_file(path: &Path, security: &mut OwnerSecurity) -> io::Result<fs::File> {
        ...
        let handle = match created {
            Ok(handle) => handle,
            Err(error) if error.raw_os_error() == Some(ERROR_ALREADY_EXISTS as i32) => open_handle(
                path,
                GENERIC_READ | GENERIC_WRITE | READ_CONTROL,
                OPEN_EXISTING,
                FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OPEN_REPARSE_POINT,
    ...
    mod tests {
        ...
        fn rejects_a_foreign_owner_sid() {
            ...
            assert!(
                validate_acl(descriptor, world.as_mut_ptr().cast(), acl, expected_owner,).is_err()
            );
        }
    }
}
