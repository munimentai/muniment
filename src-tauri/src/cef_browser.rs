...
use tauri_runtime_cef::{
    allocate_devtools_message_id, Cef, DevToolsProtocol, SandboxPolicy, SecretStorage,
    WebviewCefExt,
};
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        *control.active.lock().unwrap() = label.clone();
        for name in ["browser", "artifact"] {
            if let Some(view) = app.get_webview(name) {
        ...
        let storage = app.state::<BrowserStorage>();
        let creating = app.get_webview(&label).is_none();
        // The first CEF navigation must name the artifact. An asynchronous blank
        // page load can otherwise replace a navigation queued during creation.
        if creating && label == "artifact" && artifact_id.is_none() {
            return Ok(());
        }
        if creating {
            let name = label.clone();
            let state = control.inner().clone();
            let events = app.clone();
            let allowed_origin = storage.origin.clone();
            let url = if label == "artifact" {
                format!(
                    "{}/{}/{}",
                    storage.origin,
                    storage.token,
                    artifact_id.as_deref().unwrap_or("blank")
                )
            } else {
                "https://example.org".into()
            };
            let builder =
                WebviewBuilder::new(&label, WebviewUrl::External(url.parse().map_err(err)?))
                    .data_directory(storage.root.join("profiles").join(&label))
                    .on_navigation(move |url| {
                        state.grants.lock().unwrap().remove(&name);
                        let _ = events.emit_to(
                            "main",
                            "browser-page-change",
                            json!({"view":name,"url":url.as_str()}),
                        );
            ...
            view.on_dev_tools_protocol(move |protocol| {
                ...
                {
                    if let Some(tx) = pending.pending.lock().unwrap().remove(&message_id) {
        ...
        let view = target(&app, &label)?;
        if label == "artifact" && !creating {
            if let Some(id) = artifact_id {
                artifact_path(&storage.root, &id)?;
                view.navigate(
                    format!("{}/{}/{}", storage.origin, storage.token, id)
                        .parse()
                        .map_err(err)?,
                )
                .map_err(err)?;
        ...
        view.set_position(LogicalPosition::new(b.x, b.y))
            .map_err(err)?;
        view.set_size(LogicalSize::new(b.width, b.height))
            .map_err(err)?;
...
pub fn runtime(root: &std::path::Path) -> Cef {
    Cef::default()
        .command_line_arg("force-renderer-accessibility", None::<String>)
        .sandbox(SandboxPolicy::Required)
        .secret_storage(SecretStorage::System)
        .root_cache_path(root.join("profiles"))
