...
| ADR 0012 runtime service | The accepted design trusts one per-user service managed by the OS and the same attach peer checks. The landed ADR 0009 per-profile instance lock prevents another listener from binding the profile endpoint. The dormant `muniment-runtime` scaffold opens no endpoint, journal, CAS, or Pi. The desktop still owns the runtime today. The service boundary remains unimplemented. | No service identity exists to mint today. The design defines no privileged username or client-claimed role. The [ADR 0012 extraction amendment](docs/decisions/0012-user-level-runtime-service.md#amendment--2026-08-04-runtime-service-extraction-sequence) records the staged handoff. |
