...
| Windows | `\\.\pipe\Muniment\attach-v1-<user-hash>-<suffix>`; `user-hash` is the first 128 bits of SHA-256 over the current user SID's canonical bytes, hex encoded, and `suffix` is 128 random bits the runtime writes once to the owner-only file `%LocalAppData%\ai.muniment.desktop\attach-pipe-name`. A client reads the name from that file, so another user cannot compute it and create the pipe first. | Create the pipe with `PIPE_REJECT_REMOTE_CLIENTS` and a protected DACL granting only the current logon user SID (not a broad Users group). Impersonate each connected client, read its `TokenUser`, require an exact SID match with the desktop process token, then revert before parsing data. |
