...
Here is the honest limit. This shows the order is implementable, and our tests exercise it against real Windows pipes with both ends in one user session. It does not prove the four-byte read is harmless against a hostile process running as the same user. We claim no such defense, because same-user isolation sits outside this boundary. We measured no performance effect, because there is nothing here worth measuring.
