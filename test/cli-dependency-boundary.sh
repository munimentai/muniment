...
unexpected=$(printf '%s\n' "$packages" | grep -Ev \
  '^(muniment-cli|muniment-attach|muniment-code-diff|getrandom|cfg-if|libc|r-efi|wasip2|wit-bindgen|serde|serde_core|serde_derive|serde_json|uuid|itoa|memchr|proc-macro2|quote|syn|unicode-ident|zmij)$' || true)
