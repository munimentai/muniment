...
describe('installed local-mode chat', () => {
  ...
  it('starts a reply without cloud sign-in', async () => {
    ...
    expect(await browser.execute(async () => window.__TAURI__.webviewWindow.WebviewWindow.getByLabel('launcher').then((window) => window.isVisible()))).toBe(false)
    await browser.execute(async () => window.__TAURI__.core.invoke('launcher_open'))
    ...
    await browser.waitUntil(async () => !(await browser.execute(async () => window.__TAURI__.webviewWindow.WebviewWindow.getByLabel('launcher').then((window) => window.isVisible()))))
